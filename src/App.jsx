...
import { 
  ...
  MessageSquare, Sun, Moon, Menu, ChevronRight
} from 'lucide-react'
...
function App() {
  ...
  async function selectLesson(lesson, mode = 'view') {
    ...
    if (mode === 'edit') { setIsEditing(true); setEditTitle(lesson.title); setEditContent(lesson.content); } 
  ...
  async function handleSaveLesson(e) {
    ...
    try {
      const lessonData = { title: editTitle || new Date().toLocaleDateString('pt-BR'), content: editContent || '<p><br></p>', student_id: selectedStudent.id };
      if (isEditing) {
      ...
      {!session ? (
        ...
        <div className="flex h-screen w-full overflow-hidden relative">
          ...
          <aside className={`${isSidebarOpen ? 'lg:w-72' : 'lg:w-16'} ${isMobileMenuOpen ? 'translate-x-0 w-[280px]' : '-translate-x-full lg:translate-x-0 w-0 lg:w-auto'} transition-all duration-300 ease-in-out fixed lg:relative inset-y-0 left-0 flex flex-col justify-between bg-[var(--bg-sidebar)] text-white z-50 border-r border-[var(--border-color)] shrink-0 shadow-2xl lg:shadow-none`}>
            {!isSidebarOpen && window.innerWidth >= 1024 ? (
              ...
              <div className={`flex flex-col flex-1 justify-between overflow-hidden ${!isMobileMenuOpen && window.innerWidth < 1024 ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <nav className="space-y-3">{profile?.role === 'teacher' && <button onClick={() => {setSelectedStudent(null); setSelectedLesson(null); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-4 px-4 py-3 rounded-2xl transition-all font-semibold text-sm whitespace-nowrap ${!selectedStudent ? 'bg-[var(--bg-card)] text-[var(--text-main)] shadow-md' : 'text-white/70 hover:bg-white/10 hover:text-white'}`}><LayoutDashboard size={18}/> Dashboard</button>}
                {profile?.role === 'student' && <div className="w-full flex items-center gap-4 px-4 py-3 rounded-2xl bg-[var(--bg-card)] text-[var(--text-main)] font-semibold text-sm shadow-md"><BookOpen size={18}/> My Classes</div>}</nav></div>
          ...
          <main className="flex-1 flex flex-col h-screen overflow-hidden relative bg-[var(--bg-app)]">
            {profile?.role === 'teacher' && !selectedStudent ? (
              ...
              <div className="flex flex-col gap-4">{students.map(s => (
                <div key={s.id} onClick={() => {setSelectedStudent(s); fetchMyLessons(s.id); if(window.innerWidth < 1024) setIsLessonListOpen(true)}} className="bg-[var(--bg-card)] border border-[var(--border-color)] p-4 md:p-5 rounded-2xl hover:shadow-md cursor-pointer transition-all flex items-center justify-between group shadow-sm relative overflow-hidden">
                  ...
                  <div className={`absolute lg:relative z-40 h-full top-0 left-0 lg:left-auto ${isLessonListOpen ? 'translate-x-0 w-[280px] lg:w-[320px]' : '-translate-x-full lg:translate-x-0 w-[280px] lg:w-16'} transition-transform duration-300 ease-in-out border-r border-[var(--border-color)] flex flex-col bg-[var(--bg-card)] shrink-0`}>
                    {!isLessonListOpen && window.innerWidth >= 1024 ? (
                      ...
                      <div className={`flex flex-col flex-1 h-full ${!isLessonListOpen && window.innerWidth < 1024 ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}>
                        ...
                        {profile?.role === 'teacher' && <button onClick={() => { setIsCreating(true); setIsEditing(false); setEditTitle(new Date().toLocaleDateString('pt-BR')); setEditContent(''); if(window.innerWidth < 1024) setIsLessonListOpen(false); }} className="flex items-center gap-1 text-[#5A77DF] font-bold text-xs bg-[#5A77DF]/10 hover:bg-[#5A77DF]/20 px-2.5 py-1.5 rounded-lg transition-all border border-[#5A77DF]/20"><Plus size={14} /> Nova</button>}</div>
                        {lessons.map(l => (
                  ...
                  <div className="flex-1 flex flex-col bg-[var(--bg-card)] relative overflow-hidden transition-all duration-300">
                    <div className="w-full h-full flex flex-col">
                      {(isCreating || isEditing) ? (
                        ...
                        <div className="flex flex-1 overflow-hidden relative">
                          <div className="flex-1 p-6 md:p-12 overflow-y-auto relative z-10 pt-8 lg:pt-12"><div className="max-w-4xl mx-auto pb-10"><h2 className="text-3xl md:text-5xl font-bold text-[var(--text-main)] mb-8 md:mb-10 tracking-tight leading-tight">{selectedLesson.title}</h2><div className="text-[var(--text-main)] text-base md:text-lg leading-relaxed custom-render" dangerouslySetInnerHTML={{ __html: selectedLesson?.content || '' }} /></div></div>
